...
export default function MindfulChatParticles() {
  ...
  useEffect(() => {
      ...
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (data[(y * canvas.width + x) * 4 + 3] > 128) {
          return {
            ...
            color: "white",
            scatteredColor: "#2DD4BF", // Teal color
    ...
    function animate(scale) {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height)
      ctx.fillStyle = "#134E4A" // Dark teal background
  ...
  return (
    <div className="relative w-full h-[30vh] flex flex-col items-center justify-center">
      <canvas
        ref={canvasRef}
        className="w-full h-[45vh] absolute top-0 left-0 touch-none"
      ...
      <div className="absolute bottom-[-45px] text-center z-10">
        <p className="font-mono text-gray-400 text-xs sm:text-base md:text-sm ">
          Join the{" "}
          <a href="#" className="invite-link text-gray-300 hover:text-cyan-400 transition-colors duration-300">
            Your Mental Health
          </a>{" "}
          <span className="transition-colors duration-300"> Companion</span> <br />
          <a href="#" className="text-gray-500 text-xs mt-2.5 inline-block">
            (❤️)
          </a>
          <style>{`
